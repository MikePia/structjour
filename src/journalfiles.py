...
class JournalFiles:
    ...
    def __init__(self, indir=None, outdir=None, theDate=None, infile='trades.csv', mydevel=False):
        ...
        :param: indir:      The location of the input file (DAS Trades export). This is
                            required. (except for development verions using mydevel = True)
        :param: outdir      The name of the output directory. By default, this will be
                            indir/'out'. Structjour willalso place copied charts in this directory.
        :param: theDate:    The date of the transactions in the input file. (Currently DAS Pro
                            Trades export file only)
    ...
    def setMyParams(self, indir, outdir, infile):
        ...
        self.indir = indir if indir else self.theDate.strftime(
            r"C:\trader\journal\_%m_%B\Week_" + weekNo + "\_%m%d_%A")
        self.inpathfile = os.path.join(self.indir, self.infile)
