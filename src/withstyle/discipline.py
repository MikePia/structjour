...
class DisReqCol(object) :
    def __init__(self, thedate=datetime.date.today()) :
        ...
        rcvals = ["Date", "Time", "Long/Short", "Ticker", "Entry Price", "Account Balance", "Position", "Stop Loss", "Target", "Average Exit", "P / L", "Trade Notes"]
        rckeys = ["date", "time", "side", "symb", "entry1", "acctbal", "shares", "stoploss", "targ", "avgexit","pl" ,"notes"]
    ...
    for i in range(1,9) :
        ...
        if trade[ex].unique()[0] :
            ...
            try :
                ...
                print("Found empty shares in an exit-- this may be an overnight trade or something: Fix this bug developer!!")
            product = product + (x * p)
...
def getDevelDailyJournalList(prefix, begin):
    ...
    while now >= theDate:
        ...
        filename="{}{}/Week_{}/{}".format(prefix,
                                       ...
                                       weekCount,
                                       theDate.strftime("_%m%d_%A/out/Trades_%A_%m%d.xlsx")  
                                         )
...
def registerTrades(tsList, wb) :
    for fname, theDate in tsList:
        ...
        for row in tlog.iter_rows() :
            ...
            if ix >= 0 :
                tdf=ldf[ix]
                if not gotAnyExits(tdf): continue
                # print(cols.keys())
                ...
                formula="=$L$3+SUM($M$7:$M{})".format(row[0].row-1)
                tlog[cell] = formula
